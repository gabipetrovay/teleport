...
`examples/aws/terraform/ha-autoscale-cluster` section of the Teleport repo](https://github.com/gravitational/teleport/blob/branch/v(=teleport.major_version=)/examples/aws/terraform/starter-cluster/README.md)
...
The AWS region to run in. You should pick from the supported list as detailed in the [README](https://github.com/gravitational/teleport/blob/branch/v(=teleport.major_version=)/examples/aws/terraform/starter-cluster/README.md). These are regions which support [DynamoDB encryption
...
A `micro` instance is fine for testing, but if this server will need to support more than a few users we recommend using a bigger instance.
...
If set to the string `true`, Teleport will use [TLS routing](https://goteleport.com/docs/architecture/tls-routing/) to multiplex all traffic over a single port.
...
If using Let's Encrypt, no load balancer is configured and traffic goes straight to the instance.
...
Teleport's web interface will be available on port 443 - [https://teleport.example.com](https://teleport.example.com).
...
<Tabs>
  ...
  - Teleport's proxy SSH interface will be available via the cluster's hostname on port 3023.
  This is the default port used when connecting with the `tsh` client and will not require any additional configuration.

  - Teleport's tunnel interface will be available via the cluster's hostname on port 3024. This allows trusted clusters and nodes
  connected via node tunneling to access the cluster.

  - Teleport's Kubernetes interface will be available via the cluster's hostname on port 3026.
...
1 - Use the key name and server IP to SSH into the auth server. Make sure that the AWS keypair that you specified in the
...
<Tabs>
  ...
  ```code
  # On the Teleport server instance
...
<Tabs>
  <TabItem scope={["oss"]} label="Teleport Community Edition">
  ```code
  # When logging in with tsh, the https:// at the beginning of thh URL is not needed
...
<Tabs>
  ...
  ```code
  [ec2-user@ip-172-31-29-119 ~]$ systemctl status teleport-acm.service
  ...
  Sep 21 20:43:17 ip-172-31-29-119.ec2.internal teleport[2483]: 2023-09-21T20:43:17Z [SESSION:N] INFO Stopping session session_id:ca17914c-e0d5-42c1-9407-fa7a5c2dc163 srv/sess.go:784
