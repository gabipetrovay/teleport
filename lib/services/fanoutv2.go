...
func (f *FanoutV2) NewWatcher(ctx context.Context, watch types.Watch) (types.Watcher, error) {
	...
	w := &streamWatcher{
		...
		// before signalling watcher closure.
...
// fanoutV2Stream is a stream.Stream implemenation that streams events from a FanoutV2 instance. It handles filtering
...
func (s *fanoutV2Stream) waitInit(ctx context.Context) (types.Event, error) {
	confirmedKinds, err := s.init.wait(s.closeContext)
	if err != nil {
