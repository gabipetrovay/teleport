...
func (cfg *ClusterExternalCloudAuditWatcherConfig) CheckAndSetDefaults() error {
	...
	if cfg.Log == nil {
		...
		cfg.Log.WithField("resource-kind", types.KindClusterExternalCloudAudit)
	}
...
func (w *ClusterExternalAuditWatcher) newWatcher(ctx context.Context) (backend.Watcher, error) {
	// TODO(tobiaszheller): can I use just backend like that?
	watcher, err := w.Backend.NewWatcher(ctx, backend.Watch{
		Name:            types.KindClusterExternalCloudAudit,
		MetricComponent: types.KindClusterExternalCloudAudit,
		Prefixes:        [][]byte{backend.Key(clusterExternalCloudAuditPrefix)},
	})
	if err != nil {
		return nil, trace.Wrap(err)
	}
