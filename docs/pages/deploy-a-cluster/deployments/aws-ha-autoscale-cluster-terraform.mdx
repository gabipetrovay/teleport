...
This guide is designed to accompany our [reference ha-autoscale-cluster Terraform code](https://github.com/gravitational/teleport/tree/branch/v(=teleport.major_version=)/examples/aws/terraform/ha-autoscale-cluster#terraform-based-provisioning-example-amazon-single-ami)
...
The AWS region to run in. You should pick from the supported list as detailed in the [README](https://github.com/gravitational/teleport/blob/branch/v(=teleport.major_version=)/examples/aws/terraform/ha-autoscale-cluster/README.md). These are regions which support [DynamoDB encryption
...
<Tabs>
  ...
  - Teleport's Kubernetes interface will be available via the same network load balancer with
...
2 - Get the IP of an auth server by using `terraform output -json auth_instance_private_ips` and piping to `awk` to extract the first IP.
...
The `systemd` service name for the proxy is different between Let's Encrypt (`teleport-proxy.service`) and ACM (`teleport-proxy-acm.service`).
...
As mentioned above, the easiest way to quickly join new resources to your cluster is to use the "Enroll New Resource" wizards in the Teleport web UI.

To manually join Teleport Nodes to your cluster, you will need [a node join token](../../agents/join-services-to-your-cluster/join-token.mdx).

You should join your nodes using the public facing proxy address - `teleport.example.com:443` for our
example.
...
To add a Trusted Cluster, you'll need the public facing proxy address.
...
to an auth server. Follow the instructions in our [Trusted Clusters guide](../../management/admin/trustedclusters.mdx).
